...
# =========================================================
# v3.1 — REWARD-ONLY tweaks on top of v3, based on the
# newcode_v2 eval (5000 steps, ZERO crashes, survived 3
# in-episode lane changes — both v3 structural fixes worked):
#
# REMAINING ISSUE 1: left lane (-1.6) is a limit cycle. The
# car sweeps +2.2 -> -2.1 -> +2.2 with a ~200-step period,
# passing through the target with lateral velocity instead
# of settling. FIX: r_damp — a bonus for arriving at the
# target SLOWLY (low |cte_rate| when lane_error < 0.5).
#
# REMAINING ISSUE 2: the throttle head went dead (raw
# throttle < 0.05 on 95% of steps) because the speed bonus
# only fired at lane_error < 0.30 and the 0.10 clip floor
# made zero-output free. FIX: small always-on speed term
# (0.05 * speed) plus the gated bonus.
#
# Obs and action spaces are UNCHANGED from v3, and SAVE_NAME
# is kept as "ppo_lane_scratch_v3" on purpose: this script
# RESUMES from your existing zip instead of starting over.
# Run it in the same directory as ppo_lane_scratch_v3.zip.
#
# ----- original v3 notes below -----
#
...
TRAIN_STEPS = 300_000                 # fresh CNN policy; 100k was
                                      # thin, budget 300k+
...
class LaneTargetWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        # NEW (v3.1): damping bonus — reward ARRIVING SLOWLY.
        # Passing through the target at high lateral speed
        # earns much less than settling onto it. This is the
        # anti-limit-cycle term for the left lane.
        if lane_error < 0.5:
            r_damp = 0.4 * max(0.0, 1.0 - abs(self._cte_rate) / 0.10)
        else:
            r_damp = 0.0

        # near-wall drift-arrest bonus: reward LOW lateral velocity
        # whenever close to either wall, regardless of lane error
        if abs(cte) > 2.0:
            r_damp += 0.3 * max(0.0, 1.0 - abs(self._cte_rate) / 0.08)

        # NEW (v3.1): always-on speed term so the throttle
        # head has a gradient even when tracking is imperfect;
        # bigger bonus still gated on good tracking
        r_speed = 0.05 * speed
        if lane_error < 0.30:
            r_speed += 0.12 * speed
...
if TRAIN_STEPS > 0:
    if os.path.exists(save_zip):
        ...
        print("🆕 Fresh network — no v3 zip found, starting from scratch")
        print("   (if you meant to resume, put ppo_lane_scratch_v3.zip here)")
        model = PPO(
